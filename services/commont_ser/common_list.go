...
func CommonList1[T any](model T, option ListOption) (list []T, count int, err error) {
	var DB *gorm.DB
	if option.Debug {
		DB = global.DB.Session(&gorm.Session{Logger: global.MysqlLog})
	} else {
	...
	if ok {
		for _, db := range preloadWhere {
			l := db[1:]
			interfaceValues := make([]interface{}, len(l))
			for i, v := range l {
				interfaceValues[i] = v
			}
			query = query.Preload(db[0], interfaceValues...)
		}
	}
